...
public class LevelGen {
   ...
   public byte[] generateMap() {
      ...
      for(y = 0; y < x; ++y) {
         ...
         float z1 = this.random.nextFloat() * (float)h;
         ...
         for(int l = 0; l < length; ++l) {
            ...
            z1 = (float)((double)z1 + Math.cos((double)dir1) * Math.cos((double)dir2));
            ...
            for(int xx = (int)(x1 - size); xx <= (int)(x1 + size); ++xx) {
               for(int yy = (int)(y1 - size); yy <= (int)(y1 + size); ++yy) {
                  for(int zz = (int)(z1 - size); zz <= (int)(z1 + size); ++zz) {
                     float xd = (float)xx - x1;
                     float yd = (float)yy - y1;
                     float zd = (float)zz - z1;
